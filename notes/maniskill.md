...
ManiSkill 是一个建立在 SAPIEN 物理仿真器之上的**操作技能（manipulation skill）benchmark**，
专门针对"机器人用机械臂去操作各种**关节物体**（articulated objects，比如有抽屉、有门、有把手的家具）"
这件事，提供了一组标准化任务、3D 资产、demonstrations 和评测协议，
让不同算法可以在**同一把尺子**下比较，并且强调**对没见过的新物体的泛化能力**。

## 这是个什么场景 — 日常类比

想象你在教一个新员工"开抽屉、开柜门、推椅子、开微波炉"这些动作。
如果只让他练**家里那一个抽屉**，他可能熟练得飞起，但去到客户家面对一个**长得不一样的抽屉**就懵了。

真正想要的是：**练完一批抽屉之后，遇到新抽屉也能上手**。

ManiSkill 干的事，就是把这种"一类操作动作 + 一堆形状不同的同类物体"打包成一个考场：
- 给你 N 个不同形状的抽屉/柜门/椅子/水龙头
- 告诉你"目标是把它打开/推到那个位置"
- 划好训练集和测试集（**测试集里是没见过的实例**）
- 比赛谁的策略在新物体上做得好

不再是"我自己造个 demo，自己说我 SOTA"。
...
把"操作关节物体"从**单个 demo 任务**升级为一个**带训练/测试划分的 benchmark**，关键设计是：

1. **以技能（skill）为单位组织任务**，每个技能（比如 OpenCabinetDoor、PushChair、MoveBucket）
   下面挂**一批形状不同的具体物体实例**
2. **强制 train/test split 在物体实例层面**：训练时见过的不会出现在测试集里，
   逼算法学"这一类技能"，而不是死记某一个物体的几何
3. 提供**点云 / RGBD / state 多种 observation**，方便不同流派（RL、IL、3D 视觉）都能上
4. 配套一批**人类示范（demonstrations）**，支持模仿学习 / offline RL 等不只跑纯 RL 的研究路线
