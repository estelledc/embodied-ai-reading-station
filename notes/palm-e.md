...
把图片、机器人状态和指令拼成一段"夹着图的句子"喂给一个超大聊天 AI，让它直接吐出"机器人下一步该做什么"。
...
想象你家里来了个新管家机器人。你瘫在沙发上喊一句：

> "我把饮料洒了，能帮我擦一下吗？"

你期待的不是它回你一句"好的，主人"，而是它真的能：
...
- **会看 + 会想**：看懂场景，知道海绵在哪、桌上还有什么。
- **会规划长程任务**：把"擦水"拆成 5-6 个具体动作，一步一步往下做，做错了还能纠错。

2023 年初，市面上的方案大致两条路：

- **路线 A：让聊天 AI 出主意，让另一个机器人模型动手。** 像两个不互通语言的部门——脑子写英文备忘录，手只懂中文，中间得有个翻译。
- **路线 B：让机器人模型自己学会动手 + 看世界。** 但这种模型脑容量小，长程规划基本不行。

PaLM-E 想做的事，就是把"超大聊天 AI 的脑子"和"能看图的眼睛"和"机器人的手脚"**缝成一个端到端的整体**——你说一句话 + 给它看一眼现场，它直接吐一长串机器人能照做的步骤。
...
整篇论文做了 4 件事：定义"多模态句子"、设计多种眼睛、决定冻不冻 LLM、用一锅杂烩数据共训。

### 1. 多模态句子：把图、状态都翻译成"假装是单词"的向量

**类比**

你写日记的时候，本来全是文字。某天你想加一张照片进去——传统做法是"贴在文字下面"。但 PaLM-E 的做法更激进：

> 把照片**压缩成几个特殊符号**（比如 `<img1>`、`<img2>`）直接嵌到句子中间，让你读日记的时候一边读字一边在脑子里"看到"那张图。
