...
给机器人造了一个能拉抽屉、开柜门、拧瓶盖的虚拟"宜家家居展厅"，里面 2,346 个家具每个零件都能动。
...
想象你在家里要做一件最普通的事：从冰箱里拿瓶牛奶。

人做这事，眼睛和手会自动配合：

> 看到冰箱→看到门把手→伸手抓住→拉开门→看到牛奶→拿出来→关门。

但如果让一个机器人来做，每一步都不简单。最关键的难点不是"识别冰箱"——是要知道**冰箱的门是会动的、把手在哪、往哪个方向拉、拉多远才算开**。

2020 年之前，研究机器人的人主要在两种世界里训练算法：

- **导航世界**（如 Habitat、Gibson）：场景是真房子的 3D 扫描，但所有家具都是"焊死"的——机器人能在房间里走，但不能开门、不能拉抽屉。
- **抓积木世界**（如 OpenAI Gym + MuJoCo）：物理引擎很硬核，但场景非常贫瘠——一根杆、一个木块，谈不上什么家具。

第三类（AI2-THOR、VirtualHome）算"半交互"：机器人靠近冰箱可以触发"门打开"，但这个开门是**用代码切换状态**，不是真有把手、真有铰链、真有摩擦力。一旦你把这种环境训出来的策略搬到真机器人，立刻原形毕露——它根本没学过"如何用手指捏住把手"。

SAPIEN 想做的事，是把"导航世界的场景丰富度"和"抓积木世界的物理真实度"缝起来：每件家具都是**零件级别**可动的，每个零件都有真实的铰链、滑轨、摩擦、限位。机器人在里面学会的"开抽屉"是真的开——能直接迁移到现实。
...
想象你买了一套乐高的"动态家具"套装——柜子的门、抽屉的滑轨、瓶盖的螺纹，每个零件都用真正的小铰链连着。你拉一下抽屉，它有摩擦、会停在限位上、会被重力拉回来一点。

SAPIEN 的物理引擎做的就是把这套乐高在电脑里"完全模拟"出来。

**它在干什么**

- 用 NVIDIA 的开源物理引擎 **PhysX 4.1** 做底层。PhysX 比 MuJoCo 简单、比 PyBullet 性能更好，且免费开源——意味着任何实验室都能直接拿来用。
- 提供三种"零件连接方式"：
  - **运动学关节**（kinematic joint）：当作"无视小力的死物"——大冰箱、墙角的橱柜，不希望机器人碰一下就晃，就用这种。
  - **动力学关节**（dynamic joint）：能受力反应，但精度要求不高，比如普通的橱柜门。
...
想象有人给你一份《全国家具说明书大全》——每件家具不仅有图，还逐零件标注了"这扇门是绕轴 1 旋转的，最大开 110 度""这个抽屉沿轴 2 平移，最长拉出 0.4 米"。

光有这个说明书还不够——你还要把每件家具的 3D 模型 + 纹理 + 重量 + 摩擦力打包好，让物理引擎一加载就能跑。这就是 PartNet-Mobility 数据集做的事。
...
- 玩游戏的视角：贴图渲染，60 帧不卡，但光影细节像 PS3。
- 拍电影的视角：光线追踪，每帧渲半小时，但能看清玻璃折射、桌面反光。
...
机器人要开柜门，第一步是看着一个柜子的图，圈出"哪是门、哪是抽屉、哪是把手"。这一步叫**可动零件检测**。
...
机器人长出眼睛只是第一步，还得长出会动的手。SAPIEN 用"飞行夹爪"（Kinova Gripper 3，可以在空间任意移动的简化机械手）跑了两个具体任务：

- **抽屉拉出**：在 108 个柜子上训练。
- **柜门打开**：在 77 个柜子上训练。

成功的标准是把目标关节移动到运动范围的 90%。
