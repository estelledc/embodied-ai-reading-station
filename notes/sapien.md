...
下图是 SAPIEN 的整体架构——三大组件（引擎/数据/渲染）如何拼成"零件可动"的仿真平台：

```
   ┌──────────────────────────────────────────────────────────┐
   │  数据层 PartNet-Mobility (内容池)                          │
   │  2,346 个模型 · 46 类 · 14,068 个可动零件 · URDF+纹理      │
   │  每零件标注运动类型:  铰链○转 / 滑轨↔移 / 螺旋⟳转+移       │
   │                       + 运动限位(门≤110°, 抽屉≤35cm)       │
   └───────────────────────────┬──────────────────────────────┘
              加载 URDF (凸分解后) ▼
   ┌───────────────────────────┴──────────────────────────────┐
   │  物理引擎层 SAPIEN Engine (PhysX 4.1, 5000Hz)             │
   │  三档关节:  运动学(死物) / 动力学(受力) / articulation(力控)│
   │  ROS 接口 → 仿真代码≈真机代码 · 同步(训练)/异步(调试)      │
   └──────────┬──────────────────────────────┬────────────────┘
      物理状态 │                    渲染场景几何 │
              ▼                                ▼
   ┌────────────────────┐      ┌──────────────────────────────┐
   │ 任务层             │      │ 渲染层 SAPIEN Renderer        │
   │ ①感知: 认哪些零件动│◄─────│ 快: OpenGL 4.5 延迟光照(700Hz)│
   │  (Mask R-CNN/点云) │ RGB-D│ 真: OptiX 光线追踪(慢但精确)   │
   │ ②交互: 拉抽屉/开门 │ 法向 │ 一次出 RGB/法向/深度/分割     │
   │  (启发式 vs SAC RL)│ 分割 │                                │
   └────────────────────┘      └──────────────────────────────┘
```

*上图说明：数据层提供"每个零件都标好怎么动"的家具，物理引擎层用 PhysX 把它们变成真有铰链摩擦的可交互物体，渲染层按需在快/真之间切换，最上面两类任务（感知 + 交互）把整套平台用起来。*
...
下图对比交互任务的两条路线——人写规则的启发式管线 vs 端到端强化学习（SAC）：

```
        输入: 一个可动柜子 (RGB-D + 分割)  →  一只飞行夹爪 Kinova-3
                          │
          ┌───────────────┴────────────────┐
          ▼                                 ▼
   【路线A: 启发式管线 (人写规则)】   【路线B: 强化学习 SAC】
   ┌────────────────────────┐        ┌──────────────────────────┐
   │ ①点云中找把手位置       │        │ 三选一状态表示:          │
   │ ②生成抓取姿态           │        │  raw-exp  = 全零件位/速  │
   │ ③速度控制器拉到底       │        │  mobility = 目标轴/法向  │
   │  (开门再用 PBVS 追门边) │        │  visual   = RGB-D+分割   │
   └───────────┬────────────┘        │ 2/4/8/16 柜训练→新柜测试 │
               │                      └────────────┬─────────────┘
               ▼                                   ▼
      抽屉 95.3% / 门 81.8%            训练 88.7% ──► 测试 22.9%
      (拿 GT 视觉, 仍是 SOTA)          (严重过拟合训练柜把手几何)
               │                                   │
               └──────────────┬────────────────────┘
                              ▼  判定成功: 目标零件挪到运动范围 90%
      教训: ① 规则法>RL(当时)  ② 固定相机下 visual<raw (画面几乎不变)
```

*上图说明：启发式管线靠"找把手→抓→拉"的几何规则，在拉抽屉/开门上仍是当时 SOTA；强化学习 SAC 换新柜子就从 88.7% 崩到 22.9%，且固定相机下视觉状态反而不如直接读关节角——两大短板一目了然。*
