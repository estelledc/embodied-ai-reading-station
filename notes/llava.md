...
论文的实验分三组，每组回答一个不同的问题。把它们连起来看，你会发现整篇论文的主张其实只有一句话：**"练习题（指令数据）才是命根"——而三组实验就是从三个角度反复证明这句话。**

**实验一：LLaVA-Bench (COCO) 消融——指令数据到底贡献了多少？**

作者在 30 张 COCO 图、90 道题上做训练数据消融。最极端的对比：用全部 158K 指令数据训练得 85.1 分，完全不做指令微调只剩 21.5 分——**掉了 63.6 分**。这是全文最关键的一个数字：它证明 LLaVA 的能力不是来自"CLIP + Vicuna 拼起来"这个架构（架构不变，分数天壤之别），而是来自 GPT-4 生成的那套练习题。更细的消融还显示：只用对话类数据是 73.8 分，加上详细描述和复杂推理两类后涨约 7 分——三种题型互补，推理数据甚至反过来提升了对话表现。

**实验二：LLaVA-Bench (In-the-Wild) 对比——比同期开源对手强多少？**

在 24 张图、60 道更难的题（梗图、画作、户外场景）上，LLaVA 拿到 67.3% 的相对分（以"看了答题大纲的纯文本 GPT-4"为 100%），而 BLIP-2 是 38.1%、OpenFlamingo 是 19.1%。差距不是几个点，是**成倍拉开**——尤其复杂推理子项 LLaVA 81.7% 对 BLIP-2 的 32.9%。这个对比说明：BLIP-2/OpenFlamingo 不是"看不见图"，而是"看见了也不会按指令回答"——它们只会机械描述画面。指令微调补的正是这一课。

**实验三：ScienceQA——在有标准答案的学术题库上站得住吗？**

前两组评测都靠 GPT-4 打分，容易被质疑"裁判偏心"。ScienceQA 是多选题、有唯一正确答案，不需要 GPT-4 当裁判。LLaVA 微调后单独拿 90.92%，接近当时 SoTA（MM-CoT Large 的 91.68%）；再和纯文本 GPT-4 做"裁判合议"（两者答案不一致时让 GPT-4 仲裁），冲到 **92.53%** 的新 SoTA，也超过人类平均的 88.40%。

**哪些数字要打折扣看？**

- **67.3% 那组分数是 GPT-4 评的，训练数据也是 GPT-4 出的**——出题老师和阅卷老师是同一个人，天然可能偏爱"GPT-4 风格"的答案。评测集也只有 24 张图 60 道题，规模很小。
- **92.53% 是 LLaVA + GPT-4 合议的成绩**，LLaVA 单独是 90.92%，其实低于 MM-CoT Large 的 91.68%。读论文标题里的"SoTA"时要看清是谁的功劳。

*所以这一节是想说：消融实验证明能力来自指令数据（去掉掉 63.6 分），横向对比证明这套配方碾压同期开源模型，ScienceQA 证明它在客观题库上也站得住——但 GPT-4 既当老师又当裁判的评测方式，分数要打个折看。*
...
LLaVA 对应导读的 **[第 9 章：VLM 地基 (II)——从 BLIP-2 到 LLaVA](../guide/ch09-blip2-llava.md)**，是那一章的两大主角之一（另一个是 BLIP-2）。导读 Ch09 把 2022-2023 年的"桥梁设计之战"讲成一个完整故事：Flamingo 的门控交叉注意力（复杂桥）、BLIP-2 的 Q-Former（中等桥）、LLaVA 的一层线性投射（简单桥）——三条路线怎么竞争、为什么最后是最简单的 LLaVA 赢了。本笔记聚焦 LLaVA 单篇，Ch09 补充了它的两个对手的完整拆解，建议配合阅读。
...
Ch08 CLIP（教 AI"认图"）
    ↓
Ch09 BLIP-2 & LLaVA ← 你在这里（教 AI"看图说话"）
    ↓
Ch10 规划（SayCan、Code-as-Policies）
    ↓
Ch11-12 VLA（RT-1/2、OpenVLA、VLAS、MLA）
```

它还是后面 **[第 12 章](../guide/ch12-openvla-vlas-mla.md)** 的隐形地基：OpenVLA 的 Prismatic 架构本质上就是 LLaVA 的变体（视觉编码器 + 投影层 + LLM，只是输出从文本 token 换成动作 token）；VLAS 更是直接基于 LLaVA 架构改造，加上语音输入。可以说，理解了 LLaVA 的"眼睛 → 翻译器 → 嘴巴"三件套，Ch12 的两篇任务论文就已经懂了一半。

*所以这一节是想说：在 22 章导读体系里，LLaVA 是 Ch09 的核心论文，也是 Ch12 VLA 模型（OpenVLA/VLAS）的架构母版——它是从"看图聊天"通往"看图干活"的必经一站。*

---

## 思考题

**Q1：LLaVA 的"翻译器"只是一层线性投射（一个矩阵 W），而同期的 BLIP-2 用了复杂得多的 Q-Former。为什么更简单的方案反而赢了？这说明当时的瓶颈在哪里？**

<details>
<summary>提示</summary>

看消融实验：架构不变、只去掉 158K 指令数据，分数从 85.1 掉到 21.5。这说明瓶颈根本不在"桥怎么修"，而在"有没有合适的练习题"。LLaVA 把工程精力全押在数据上，用最简单的桥换来最快的实验迭代速度（论文自己说 lightweight 方案让他们能快速做 data-centric 实验）。当数据是瓶颈时，架构上的精巧设计收益有限。
</details>

**Q2：GPT-4 根本看不见图，为什么它能出"看图题"？这个数据生成流程里，图像信息是通过什么形式传给 GPT-4 的？这种转换会丢失什么？**

<details>
<summary>提示</summary>

图像被转成两种"符号化表示"喂给 GPT-4：人写的 caption（描述场景）和物体框坐标（物体类别 + 位置）。丢失的东西：caption 没写到的细节（纹理、颜色深浅、小字）、框与框之间的精细空间关系、以及一切"要亲眼看才知道"的信息。所以 GPT-4 出的题天然偏向"caption 里写了的内容"，这也解释了 LLaVA 为什么在 OCR、细粒度识别上弱。
</details>

**Q3：为什么训练要分两个阶段？如果跳过 Stage 1 直接做 Stage 2，会发生什么？论文里有什么数字支持？**

<details>
<summary>提示</summary>

Stage 1 只训投影矩阵 W，相当于先给冻结的 LLM 配一个"视觉分词器"——让图向量落到 LLM 词嵌入空间里。ScienceQA 消融显示：跳过预训练（Stage 1）从头训，准确率从 90.92% 掉到 85.81%，损失 5.11 个点。原因是图像特征还没对齐就动 LLM 的权重，会把 LLM 原本的语言能力搞乱——先校准乐器，再合奏。
</details>

**Q4：LLaVA 评测用"GPT-4 当裁判打相对分"。这套评测方式有什么系统性风险？如果让你设计一个更可信的评测，你会怎么补？**

<details>
<summary>提示</summary>

风险至少三条：(1) 训练数据是 GPT-4 生成的，裁判也是 GPT-4——模型可能只是学会了"GPT-4 喜欢的答题风格"而拿高分；(2) 裁判看到的"视觉信息"其实是文字描述，它无法核实答案里的视觉细节是否是幻觉；(3) 评测集只有几十张图，统计上很脆弱。补法：加入有客观标准答案的基准（论文自己用 ScienceQA 就是这个思路）、换不同家族的模型当裁判、做人工盲评抽查。
</details>

**Q5：论文发现 LLaVA 会把"冰箱里有草莓 + 有酸奶"回答成"有草莓味酸奶"。这个失败暴露了架构上的什么问题？后续工作可能从哪些方向修？**

<details>
<summary>提示</summary>

论文的诊断是 LLaVA 有时把图像当"一袋 patch"（bag of patches）——CLIP 输出的 256 个 patch 向量经过线性投射后，物体和属性的绑定关系变弱了，"草莓"和"酸奶"两个概念混在一起。可能的修法：更高分辨率输入（看清标签上的字）、更强的视觉编码器、在数据里加属性绑定类的练习题、或者用带空间结构的连接器而不是把 patch 摊平。LLaVA-1.5 的分辨率升级（224→336）就部分沿着这条路走。
</details>

**Q6：消融显示 13B 模型换成 7B 只掉 1.08 分（90.92% → 89.84%），但去掉指令数据掉 63.6 分。如果你只有一张消费级显卡的预算，这两个数字告诉你该怎么分配资源？**

<details>
<summary>提示</summary>

结论很直白：钱花在数据上，不是模型上。模型减半几乎不伤性能，说明这个任务对参数规模不敏感（至少在 7B-13B 区间）；而指令数据的质量和多样性是性能的决定因素。实操上：选 7B 基座省显存，把省下的算力/预算投入构造更好的指令数据。这也是 LLaVA 之后开源社区"卷数据配方不卷参数"风气的起点之一。
</details>

**Q7：LLaVA 的训练数据全部由 GPT-4 生成，这带来一个"老师天花板"问题。除了能力上限，这种"用闭源模型蒸馏开源模型"的做法还有什么合规和生态上的隐患？**

<details>
<summary>提示</summary>

合规上：OpenAI 服务条款禁止用其输出训练与 OpenAI 竞争的商业模型，所以 LLaVA 权重研究可用、商用受限。生态上：如果整个开源社区都靠蒸馏 GPT-4，那么 (1) 开源模型会系统性继承 GPT-4 的偏见和错误；(2) 开源生态的能力上限被闭源模型锁死；(3) 一旦条款收紧或 API 关闭，数据管线断供。后来社区推动"用开源模型生成训练数据"正是对这个隐患的回应。
</details>
