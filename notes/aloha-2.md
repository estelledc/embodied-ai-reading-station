...
ALOHA 2 是 ALOHA 双臂遥操作平台的硬件升级版：人体工学更顺手、零件更耐用、装配更标准化，并完整开源（CAD、BOM、固件、软件栈）。它本身不是一个新算法，而是一个"让大规模双臂示范数据可负担"的工程基座 —— 想做模仿学习（imitation learning）的实验室不必各自重造轮子。

## 这是个什么场景 — 日常类比

想象你要教一个机器人"叠衣服 / 倒水 / 拉拉链"这类需要双手配合的精细动作。最直接的方式是人来手把手示范：人捏着两只"主臂"操作，机器上的两只"从臂"会同步动，全程录下来当训练数据。

第一代 ALOHA 就是干这个的，但用过的人会反馈：用一上午手就酸了 / 这个零件三天断一次 / 复刻一台要踩半个月的坑。ALOHA 2 就像把"原型机改成量产款"—— 同样的工作原理，但你能舒服地连续录 8 小时数据，而且你的合作实验室能在两周内也搭出一台一模一样的。
...
**夹爪与握把（gripper）重新设计**：v1 用橡皮筋作为夹爪开合的被动元件，磨损快、扭矩不稳定。v2 换成低摩擦机构（具体方案需读原文，可能是张紧弹簧 / 滑轨结构），同时把主臂的握把（操作员手握的部分）做成更符合手部解剖的形状，长时间握持不易疲劳。这一改动直接把"单次连续操作时长"从分钟级推到小时级。

**结构件与传动**：v1 的部分 3D 打印件改成 CNC 铝件或注塑件，关键关节的电机座和线缆走向重新设计，减少线材在反复运动中的应力集中。整体目标是让一台机器在每天 4–8 小时使用下、跑半年不需要大修。

**视觉与工作台标准化**：摄像头位置、工作台尺寸、灯光环境都给了推荐配置（具体参数需读原文）。这件事看似琐碎，但对"跨实验室数据可合并"非常关键 —— 如果 A 实验室的桌子比 B 矮 5cm、视角偏 10°，模型在 A 学的策略到 B 就可能直接失效。

**软件与开源交付**：ROS 驱动、遥操作脚本、数据录制管线、与 ACT/Diffusion Policy 的接入示例全部开源。论文页面给出了 CAD 文件、BOM（物料清单）、装配手册、调试视频。也就是说"开源"在这里不是放个代码仓库，而是"提供整套从下单零件到跑通 demo 的路径"。
