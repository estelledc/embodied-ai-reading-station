...
BridgeData V2 是一个开源的机器人操作（manipulation）数据集，收集了约 60K 条 WidowX 机械臂的演示轨迹（demonstration trajectory），覆盖 24 个不同环境的多样任务。它是当前训练通用 VLA（Vision-Language-Action）模型时最常被引用的"事实标准"预训练底座之一——你看 OpenVLA、RT-X 之类的工作，BridgeData V2 几乎都在它们的训练混合（training mixture）里。

## 这是个什么场景 — 日常类比

想象你要教一个完全没见过厨房的新员工"做饭"。你不可能只给他看一个厨房、一个菜谱，然后期待他到任何厨房都能上手。你得让他在 24 个不同的厨房（不同灶台、不同锅具、不同光线）里看 6 万次"开抽屉、拿胡萝卜、放进锅里"这样的基础动作演示。看完这些，他至少建立了"操作"这件事的肌肉记忆，再去新厨房学新菜也快得多。

BridgeData V2 干的就是这件事：给机器人提供一个"足够杂、足够多"的操作演示库，作为它进入"现实世界"前的基础训练。
...
**硬件统一**：所有数据用同一款机械臂——WidowX 250 6-DoF 机械臂 + 第三人称 RGB 相机（具体相机数量和型号需读原文）。统一硬件意味着收来的数据可以直接拼一起训练，不用做跨本体（cross-embodiment）的归一化。

**采集方式**：以人类遥操作（teleoperation）为主，部分轨迹用脚本策略（scripted policy）+ 少量自主探索（autonomous）补充。遥操作保证数据质量"看得过去"——演示者的意图清晰，不像随机 rollout 那样动作零散。具体每种来源的占比需读原文。

**环境设计**：24 个环境分布在多个真实场景中——厨房、桌面、玩具房等。每个环境里有多个任务，每个任务里有多种物体姿态/初始条件。这样训出来的 policy 不会过拟合到"这个特定的桌子角"。

**语言标注**：每段轨迹（trajectory）配一条自然语言指令，描述"机器人在做什么"。标注既有人工写的，也有模板化生成的。这一层让数据集天然适配 BC（Behavior Cloning）+ language conditioning 的训练范式，也是后来 RT-2、OpenVLA 直接用它做预训练的原因。
