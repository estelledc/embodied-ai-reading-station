...
让 agent 先在脑子里搭一个游戏的"梦境模型"，然后整个训练过程都在自己做的梦里完成，再把学会的本事带回真游戏。
...
> 投手扔出一个 100 mph 的快球，球离开手 4 米后飞到你眼睛里——这中间只有 0.4 秒。
> 但你的眼睛把图像传给大脑、再让大脑想清楚要不要挥棒，差不多需要 0.2 秒。
> 顶级击球手是怎么做到的？他们不是"看到再决定"，而是脑子里早已有一个**预测模型**：
> "投手抬手的姿势 → 这是个外角下坠球 → 它会飞到这里"——预测先行，肌肉跟上。

这篇论文要做的事，就是给 AI agent 装一个类似的"脑内预测器"。

更现实的需求来自强化学习（RL，reinforcement learning：让 agent 通过试错学会在某个环境里得高分的方法）。当时大家训 agent 玩游戏面临两个痛苦：

- **需要海量交互**：在真环境里跑一帧就要算一帧物理 / 渲染图像，慢且贵。
- **大脑容量受限**：传统方法没法把"看图的眼睛"和"做决策的脑子"一起做大，因为给一个庞大的网络分配信用（哪一步动作错了？）非常难。

作者的小目标：**让 agent 先用便宜的"做梦"代替大部分真环境交互，并且让"看的部分"做大、"决策的部分"做极小**——这样训练既快又稳。
...
整篇论文的 agent 由三块组成：**V**（视觉）、**M**（记忆）、**C**（控制器）。前两块拼起来就是"世界模型"，第三块是真正做决策的小脑袋。

### 1. V 模块：用 VAE 把每帧图压成 32 个数字

**类比**

你给朋友打电话描述一张图："场景大概是赛道、左边有点绿草、远处有个左转弯。"——你没有把每个像素一个个念出来，而是抽出了**几十个最关键的描述维度**。VAE 干的就是这件事。
...
> **VAE（Variational Autoencoder，变分自编码器）**：一种"把图压成向量、又能从向量还原回图"的网络。和普通自编码器的区别在于它不只输出一个 z，而是输出 z 的概率分布（均值 µ + 方差 σ）。这样压出来的向量空间更"规整"，相邻向量解码出来也是相似图像。

> **潜变量 z（latent vector）**：32 或 64 维的小向量，代表一帧图的"压缩摘要"。可以理解成"这张图的指纹"。

> **KL 损失（KL loss）**：罚分项，强迫 z 的分布不要乱跑、要贴近一个标准的正态分布。这是 VAE 的关键约束。
...
你看到一个网球被抛到半空，下一刻它会落在哪里？你脑子不会回答一个确定的点，而是"大概率落在球场内 2 米半径，小概率被风吹偏到 5 米外"——一个概率云。M 模块要学的就是这种"未来的概率分布"，而不是单点预测。
...
V 和 M 像是司机的视力 + 经验直觉，C 是真正按方向盘踩油门的肌肉记忆。司机的视力可以训得很好（眼睛 4M 参数），但负责"踩多深油门"的手脚控制反而越简单越好——肌肉只听一句"现在要左多少度、加速度多少"就够。
...
棋手在脑子里复盘下一百盘棋（梦境），不需要找真对手。等他到现场比赛时，就用脑内复盘出来的那套招式。
