...
把一个会预测下一秒视频画面的大网络，再训练一轮，就让它学会指挥机器人手臂做家务。
...
想象你买了一台机器人帮你做家务，你说"把糖装进密封袋"。

这件事你做大概两秒钟。但机器人要在脑子里办完一连串事：
...
1. **物理直觉**：手伸下去会发生什么、东西会不会倒、袋子会不会瘪。
2. **动作多样性**：同一个目标，可能有十种合理的手伸过去的路线，模型要能输出其中之一，而不是卡死在一条路上。

> **机器人策略（policy）**：一个函数，输入是"眼前看到的画面"，输出是"接下来手要怎么动"。可以理解为机器人的"反应公式"。

举个学校的类比：你做物理题时，看到题面（输入），写出解题步骤（输出）。policy 干的就是这件事，只不过输入是摄像头画面，输出是关节怎么转。
...
**直接拿一个会预测下一秒视频的现成大网络（叫 Cosmos-Predict2），不加任何新部件，只是再训练一轮，让它顺便输出机器人动作。**

这就像你已经请了一位会画连环画的老师，现在只让他在画下一格的时候，**顺手在角落写一行字**告诉你"主人公接下来该往哪走"。不换老师、不加助教，就让他多干一件事。
