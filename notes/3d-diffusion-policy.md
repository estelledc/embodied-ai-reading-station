...
一句话：把"图像版的 Diffusion Policy"里那个"看图像"的眼睛换成"看一团点云"的眼睛，再加个轻量小 MLP 把点云压成 64 维向量，结果只用 10 条示教数据就能在 72 个仿真任务上平均超过原版 24.2%；真机 4 个任务、每个 40 条示教，平均成功率 85%，而且基本不会乱挥手撞硬件。

更直白的版本：

- 老办法：给机器人看 RGB 照片 → 猜动作。学不会"高度"和"远近"，得喂 100~200 条示教
- 新办法：给机器人看"3D 点云素描"（只有 xyz、没颜色）→ 猜动作。10 条示教就够，还能换桌子换灯光不掉链子
- 关键发现：3D 表征不只是"加点信息"，它还让模型变得更安全、更省数据、推理速度还差不多（12.7 FPS vs 12.3 FPS）
...
**模仿学习（imitation learning）** 就是：人先示范几遍（用遥操作让机器人手动作一遍），录下"我看到什么 → 我做了什么"，机器人学着照搬。这比强化学习省心，因为不用机器人自己摔几千次去试错。

但传统模仿学习有个老毛病：**特别费示教数据**。

- 原版 Diffusion Policy（DP，2023 RSS）：每个真实任务要 100~200 条人工演示
- 一条演示要遥操几十秒到几分钟
- 演示一多就会出错、要重来，整个数据收集能拖好几天

为什么这么费？因为机器人是**通过 RGB 图像**看世界的。RGB 图像只有"我看到一个杯子在画面左边"，不知道"杯子离我 30cm 还是 60cm"。结果：
...
DP3 这篇文章问：**如果让机器人直接看 3D（点云），能不能从这个根上解决？**
...
把 DP3 跑一次（训练阶段）拆成 6 步。

**步骤 1：拍一张深度图**

相机（如 RealSense L515）拍一张 84×84 的深度图。注意：是深度图（每个像素值是"离相机多远"），不是 RGB。

**步骤 2：深度 → 点云**

利用相机的内参（焦距等）和外参（位置朝向），把每个深度像素反投影成 3D 空间里的一个 (x, y, z) 点。

类比：深度图就像一张"等高线地图"，反投影就是把它从平面拉成立体。

**步骤 3：裁剪 + 下采样**

- **裁剪（Crop）**：用一个长方体边界框框出工作区，扔掉桌面、地面、远处墙等无关点。这步影响巨大——消融显示去掉 cropping 平均成功率从 78.3% 掉到 45.3%
- **下采样（FPS）**：从剩下的点里挑 512 或 1024 个，用最远点采样保证均匀分布

类比：原始点云像一张超高清照片，cropping 是裁框，FPS 是把分辨率压到合适大小但保留构图。
...
整个网络就是 **三层 MLP + 一次 max pool + 一个投影头**。MaxPool 让顺序无关（点云本来就是无序的）。

机器人位姿 q（关节角度等，DimRobo 维）也走一个类似的小 MLP 编码成 64 维。然后两个向量拼起来得到 128 维条件向量。

**步骤 5：把条件向量喂给扩散策略**

扩散策略（Diffusion Policy backbone）就是个**条件去噪网络** ε_θ：

- 训练时：从演示里取真实动作 a₀，加 k 步噪声变成 a_k，让网络预测"加进去的噪声 ε_k"。损失函数：MSE(ε_k, ε_θ(a_k, k, v, q))
- 推理时：从随机高斯噪声 a_K 出发，迭代 K 次去噪，每一步：a_{k-1} = α_k a_k − γ_k ε_θ(a_k, k, v, q) + σ_k 噪声，最后得到 a₀ = 一段动作序列

人话翻译这条公式：**给我当前看到的环境（v）和我自己的状态（q），帮我从"满屏雪花"里逐步擦出一段合理的动作**。

**步骤 6：执行**

预测出 H=4 步动作，但只执行最后 N_act=3 步（短 horizon 是为了在真实环境里能随时切换）。
