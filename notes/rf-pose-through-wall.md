...
用一个发射 Wi-Fi 频段电波的小盒子，隔着墙也能"画"出屋里人的骨架动画——靠的是把摄像头当老师、电波当学生学了一遍。
...
想象一栋着了火的房子。烟很浓，红外摄像头会被烟挡住，普通摄像头更别提。消防员只知道"楼上可能还有人"，但**到底在哪个房间、躺着还是站着、有没有动**，完全不知道。每多一秒搜索，被困者的存活率就掉一截。论文开头引的数据是：1994-2013 年间全球 6873 起自然灾害造成 135 万人死亡——这里面有相当一部分本来可以被"看见"。

再换一个场景：医院想监控独居老人有没有摔倒，但老人不愿意装摄像头——感觉被偷窥。

这两个场景的共同点是：**我们想知道"屋里那个人长什么姿势"，但又**：
1. 看不见（烟、墙、家具挡住了），
2. 不能让他穿设备（昏迷的人没法戴手环）。

可见光波长只有几百纳米，遇到墙就被反射或吸收了。其它能穿过墙的电磁波要么有辐射风险（X 光），要么波长太长画不出细节（长波广播）。**Wi-Fi 频段的电波**（3-10 GHz）刚好处在"既安全又能穿墙"的甜点位——波长 5 厘米左右，能穿木板、石膏板、甚至砖墙，而**人的皮肤会反射回去一部分**。这就给了我们"用电波当眼睛"的可能性。

这篇 2019 年的论文要做的事就是：**装一个能发 Wi-Fi 频段的小天线阵列，让它代替摄像头，画出屋里人的骨架。**
...
**与其让模型看一帧画一帧（像照片），不如让它看一段时间再画当前帧（像视频）——因为人在动，不同时刻不同身体部位才会反射回信号。** 加上"用摄像头当老师教电波模型"的跨模态监督，整个系统就能在没人手工标注的情况下学会穿墙画骨架。

具体落地的做法是论文起的名字 **"Many-to-Many Encoder/Decoder"**：
- **多帧输入**（many in）：连续若干帧的雷达热图同时进入网络；
- **多帧输出**（many out）：每一帧都解码出对应的骨架；
- 中间靠 **LSTM** 把过去几帧的"零碎反射"在隐藏状态里累积起来，所以第 t 帧的预测不是只看第 t 帧的输入。

对比之下，传统 RGB 姿态估计是 **one-to-one**：一张图进、一张骨架出。这两种范式的差别本质上来自**信息密度**：RGB 一张图就饱和，RF 一张图永远不够，必须时序拼接。
