...
第一次用 Transformer 直接端到端开机器人，把 13 万条人类示教数据"喂"给一个会聊天又会动手的小模型。
...
想象你刚招了一个新员工到办公室茶水间。你不想专门给他写一本厚厚的"操作手册"，只想随口说一句：

> "去把可乐罐放进中间那个抽屉。"

理想的助手应该听懂这句话、看一眼桌子、然后走过去打开抽屉、捡起可乐、放进去。**这一连串动作里其实包含了：听懂语言、识别物体、规划路线、控制手臂角度、闭合手指、关上抽屉**——任何一步出错都会失败。

可在 2022 年，机器人圈的状态大致是这样：

- **专门为某个任务训的机器人**：能把一种零件准确放进一个孔，但你换个零件它就不行了。像只会做一道菜的厨师。
- **会聊天的 AI（GPT 之类）**：满嘴跑火车，但不会动手。像只会读菜谱的美食评论家。
- **从仿真训出来的机器人**：在虚拟环境里跑得飞起，一到真实世界就摔得鼻青脸肿。像只在模拟器里学过开车的新手司机。

RT-1 想做的事是：**让一个统一的模型，听懂语言指令，看摄像头画面，直接吐出 7 个关节角度该怎么动，能在真实办公室厨房执行 700 多个任务**。
...
教小孩说话不能只让他听天气预报——他得听到妈妈的话、爸爸的话、邻居的话、电视里的话，才能学会语言。要训一个通用机器人，单一任务数据远远不够。
...
想象 Transformer 是一台只吃豆腐块的机器。你的食材有米饭、青菜、肉。怎么办？把所有东西**切成同样大小的小方块**塞进去。这个"切方块"的过程在 AI 里叫 **tokenization（标记化）**。
...
教小孩学骑自行车不是给他讲牛顿定律，而是让他先看你骑、然后扶他骑、错了纠正一下。RT-1 的训练就是这样——给它看人类示教，错了就调一调内部参数。
...
赛车手在比赛前会反复琢磨怎么减重、怎么省油、怎么少换挡。RT-1 团队为了让模型在真机器人上跑得动，花了同样多的功夫做工程。
