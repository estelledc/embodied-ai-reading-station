...
不让网络直接吐出动作，而是让它给候选动作打分，再挑分最低的那个——机器人模仿学习突然变聪明了。
...
想象你在教一个新员工切水果。你切给他看 50 次，他要照着你的样子切。问题是——同一个西瓜，你这一刀可能从左切，下一刀从右切，都对；但如果你叫他"平均一下两种切法"，他就会从中间切下去，把刀劈在西瓜的硬皮上。

这个新员工就是机器人。他面对的是一类很常见的动作：

- 同一个状态有**多个合理动作**（左推、右推都行）
- 动作之间有**断点**（推到位之前是慢慢推，到位之后立刻停下，没有中间地带）
- 有时候**精度要求 1 毫米**（把方块塞进卡扣里，太轻塞不进、太重会卡死）

传统的"看一眼 → 算出动作"网络在这种场合特别容易翻车，因为它本质上是在画一条平滑的曲线，而真实的最优动作分布既不平滑也不唯一。

IBC 想做的事，是**换一种办法让机器人决定动作**——不是直接算出来，而是先打分再挑。
...
整篇论文围绕 4 个动作展开：换公式、训练打分网络、推理时搜索、多种实现验证。
...
- 思路 A：朋友直接告诉你"去这家"（显式策略，对应 `a = F(o)`）。
- 思路 B：朋友给你一份"难吃指数表"（每家餐厅一个分数），你自己挑分数最低的那家（隐式策略，对应 `a = argmin_a E(o, a)`）。

显式策略很怕"两家都好"——朋友会卡壳或者说一家"折中的"，但折中的那家可能根本不存在。隐式策略不会卡壳，因为你看到的是分数表，两家都是 1 分、第三家 5 分，挑哪家都行。
