...
Diffusion Policy 跑得慢，是因为它要"一边煮饭一边搅 100 次"才能生成一条动作。本文用一种叫 Consistency Distillation（一致性蒸馏）的训练技巧，把这个慢锅炖过程压缩成"一勺出锅"——用一步直接从纯噪声跳到一条可执行的机器人动作。

具体做法：先用 EDM 框架训练一个标准的 Diffusion Policy 当老师；再训一个学生网络，强迫它在老师铺好的"去噪轨迹"上任意两点都能直接跳到同一终点。学生收敛后，单步推理 ≈ 1 ms，比原本 100 步 DDPM 快约一个数量级，在 6 个仿真任务和 3 个真实任务上成功率不降甚至略升。关键的工程取舍有三个：

1. 选 CTM-local 作为一致性目标函数（相邻 t/u + 任意 s）
2. 把初始噪声的方差从 N(0, T²I) 降到 N(0, I)
3. 3 步推理时把 chaining 时间点固定在"早中段"

更关键的是，作者在笔记本 GPU（3070 Ti, 8GB VRAM）上做了三个真实任务（捡垃圾、插插头、移动操作微波炉），DDiM 100 步要 1.5 秒一动作，CP 21 ms——直接把"扩散策略不能板载部署"这个老问题打掉了。

*所以这一节是想说：把扩散策略的"100 步炖煮"通过蒸馏压成"1 步出锅"，而且不输味道，还能塞进笔记本 GPU。*

## 这是个什么场景

机器人模仿学习里，你给它 200 段人类示教，希望它学会"看到桌上有杯子 → 去抓"。Diffusion Policy（2023 RSS 那篇）是当时的 SOTA，它把"动作"当成图像去做扩散生成：先随机采一段全是噪声的动作序列，再用神经网络一步步去噪，最后得到一条平滑的动作轨迹。

问题在于推理时延。打个比方，扩散模型生成动作就像冲胶卷——你必须把胶卷在显影液里来回搅 100 下，影像才能逐步浮现。在 NVIDIA T4 上，DDPM 100 步要 ≈ 1 秒；改用 DDiM 15 步也要 ≈ 11 ms。这对**笔记本 GPU 上跑的移动机器人 / 四旋翼**是致命的：

- 动态任务（接住一个滚下桌的球、跟着移动目标走）需要至少 30 Hz 控制频率，1 秒一动作完全不够用
- 板载算力受限：3070 Ti 笔记本 GPU 跑 100 步 DDPM 要 1.5 秒/步，等同于残废
- 同机器还要并行跑感知、SLAM 等其他模型，VRAM 卷不动

作者的目标：**保留 Diffusion Policy 的成功率，把推理时间砍到 ~1/10**。

补充一个具体场景：论文里的 Microwave 任务是一个移动操作（mobile manipulation）任务，机器人需要"导航到微波炉 → 开门 → 拿取一袋西兰花 → 放进微波炉 → 关门 → 按蔬菜键"。整套流程涉及移动底盘 + Kinova Gen3 7-DoF 机械臂，完全跑在笔记本 + WiFi 路由器上。如果每个动作要等 1.5 秒，机器人在导航过程中会"卡顿式前进"，肯定撞东西。这就是为什么"快"不是工程美学，而是任务可行性问题。
...
为什么这个想法能成立？关键在于"自一致性（self-consistency）"这个假设。打个比方，你想象一条登山路径，从山顶（纯噪声 t=T）到山脚（动作 x₀）。Diffusion Policy 学的是每一段路怎么走，所以要走 100 步。但如果这条路径已经被老师铺好，**路径上任意两点 (xₜ, xᵤ) 经过同一条 ODE 出发，最终一定到山脚同一个点**。学生学的是"看到我现在在路径的哪个位置，直接告诉我山脚在哪"——它不再需要逐段走。

为什么不能直接拿一个 Diffusion Policy 跑 1 步？因为 Diffusion Policy 是按"小步走"训出来的——它的网络只在"局部去噪"这件事上学得好，给它 t=T 让它一步跳到 0，结果是噪声。所以必须**重新训一个网络**，让它专门学"大步跳"。这个新网络就是学生，是本文的产物。

具体到论文，作者做了三件事：

1. **换个老师框架**：把原 Diffusion Policy 的 DDPM 老师换成 EDM 老师。原因是 DDPM 是 SDE，蒸馏需要确定性 ODE，所以必须切到 EDM/DDiM 这类。
2. **借 CTM 目标函数**：照搬 Kim 2023 的 CTM 损失（任意两点 xₜ, xᵤ 都要在 s 时刻预测出同一个 xₛ），但发现它训练慢 40%，于是改用一种折中版本"CTM-local"——只用相邻时间点 t 和 u=t-1，但允许 s 任意 < u。
3. **三个工程小改动**：见下节。
