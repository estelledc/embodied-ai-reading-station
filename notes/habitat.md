...
给"会动的虚拟机器人"造一个又快又像真房子的练功房，让它在里面练上几亿步路再上岗。
...
想象你想训练一只导盲犬。最理想的情况是：让它在真实城市街道上每天走路、找路、试错。但现实里这事行不通——

- 真实世界**只能 1 倍速**前进，一天就是 24 小时，没法快进。
- 真实街道**不可并行**：你不能让 8 只狗同时在同一条街上各自试错，不打架。
- 真实世界**危险**：训练失败的狗可能闯红灯撞车。
- 真实世界**贵**：场地、伙食、保险都要钱。
- 真实世界**没法重来**：上次走错那条路的灯光、行人、天气，再也复刻不出来。

那怎么办？做一套"VR 导盲犬训练舱"——把真实街道扫描成 3D 模型，让虚拟狗在里面练。**这就是 Habitat 在做的事**，只不过它针对的是"室内导航的虚拟机器人"。

具体场景：你跟家里的机器人说"去看看我笔记本是不是在书桌上，有就给我拿过来"。机器人要会**视觉识别**（认出书桌、笔记本）+ **语言理解**（听懂这句话）+ **室内导航**（在房间里找路、绕开沙发）。这套技能的训练成本，决定了"能造出这种机器人吗"这件事到底可行不可行。
...
整个论文做了 4 件事：底层渲染引擎、数据集解耦、任务 API、然后用这套东西做了一组以前没法做的大规模实验。
