...
BLIP-2 提出一个轻量的"翻译模块"叫 Q-Former（Querying Transformer，查询式 Transformer），它夹在两个**完全冻结**的大模型中间——一个是已经训练好的视觉编码器（比如 ViT、EVA-CLIP），一个是已经训练好的大语言模型（比如 OPT、FlanT5）——只训练这个中间小模块，就能把图像"翻译"成 LLM 能听懂的话。结果是：花极少的算力，做出了当时 SOTA 级别的视觉-语言模型，并且奠定了后来大量 VLM（视觉语言模型）"冻结大模型 + 训练小桥梁"的通用范式。

## 这是个什么场景 — 日常类比

想象你有一个**只会看不会说的画家**（视觉编码器），和一个**只会写不会看的作家**（LLM）。两人都是各自领域的大师，但语言不通。

之前的做法基本是两种：
- 把两人关起来一起重新培训——成本极高（端到端训练大模型）。
- 让作家也学会看画——还是要动作家的脑子（微调 LLM）。

BLIP-2 的思路是：雇一个小翻译（Q-Former），让两个大师都不动脑、保持原状，只让翻译反复学习"怎么把画家看到的东西转述成作家能写的话"。翻译只有几千万到一亿参数，画家和作家加起来几十亿到上百亿，但被冻住不更新——所以训练成本主要花在翻译身上，便宜很多。
...
第一，**两端都不动**。视觉编码器和 LLM 全程冻结，参数完全不更新。这样可以无脑替换上游模型——明天 EVA-CLIP 出新版、LLM 换成更强的，不用重训。

第二，**只训中间一个 Q-Former 小模块**。Q-Former 内部有一组**可学习的 Query 向量（Query Embeddings，可学习的查询向量）**，数量很少（论文常用 32 个）。这些 Query 像"提问者"，通过 cross-attention 反复向冻结的图像特征"提问"，把一张图压缩成 32 个语义向量。

第三，**两阶段训练**。第一阶段把 Q-Former 接到视觉编码器上，做表征学习（图文对比 + 图文匹配 + 图像-文本生成三个任务联合）；第二阶段把 Q-Former 输出的 32 个 Query 当作"软 prompt"喂给 LLM，让 LLM 在冻结状态下做生成式预训练。两阶段把对齐和生成解耦，避免一锅炖学不动。
