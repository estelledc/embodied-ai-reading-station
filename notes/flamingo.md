...
DeepMind 教一个聊天 AI 看图，并且看几张例题就能学会新任务。
...
设想你转学到一所新学校，老师上课没解释规则，直接指着黑板说：

> "看这道题，答案是 A；这道题答案是 B。下一道，你来。"

你光看了两三个例子，就能猜出题型怎么做——人类很擅长这种"举一反三"。

GPT-3 在文字世界里第一次让 AI 也能这样做：你给它三五个"问 → 答"示范，它就会照着规则答下一题，不需要专门训练。这种本事叫**少样本学习（few-shot learning）**。

但 2022 年初，**带眼睛的 AI 还做不到这件事**。如果你想让一个看图 AI 学会一个新任务，比如"看医学影像写报告"，常规做法是收集几千上万张标注图，再训练几小时甚至几天。每换一个任务，整套流程重来。

Flamingo 想做的事就是：**给视觉模型也装上 GPT-3 那种"看几个示范就会"的本事**。你只要在提示里塞几对（图、答案）示范，再丢一张新图，它就照葫芦画瓢。
...
整篇论文做了 4 件事：把图压成少量 token、把视觉信息塞进 LLM、处理图文交错序列、在网页规模数据上训练。
