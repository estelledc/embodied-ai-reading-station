...
让 AI 在脑子里"做白日梦"练打游戏，第一次靠纯想象就在 Atari 上打到人类水平。
...
- **方案 A（model-free）**：每天上棋盘真实对弈几百盘，输了改、赢了记。慢，但直接。
- **方案 B（model-based）**：先在脑子里建一个"棋盘模拟器"——它能预测"我下这步、对方大概会怎么应"。然后大部分时间在脑子里推演，偶尔上真棋盘验证。

人类高手其实是后者：闭眼也能下盲棋，因为脑子里有棋盘的"内部模型"。

强化学习里的 Atari（雅达利老式街机游戏，比如打砖块、太空入侵者）一直是 AI 的算盘——50 多个游戏，每个目标不同，用来检验通用性。问题是：**之前所有"在脑子里建模拟器"的 AI 在 Atari 上都输给"直接打游戏"的 AI**。

DreamerV2 第一次在 Atari 上把"脑内模拟"这条路走通了。

> **强化学习（RL）**：让 AI 通过"试错 + 拿奖励"自己学会一件事的训练方法。比如让它打游戏，赢了加分、输了扣分，慢慢学会怎么打分高。
>
> **Atari 基准**：1980 年代街机游戏组成的标准测试集。AI 圈用它来比较谁的算法更通用——同一套代码能不能把 55 个完全不同的游戏都打好。
...
整个 DreamerV2 由三件事组成：学世界模型、在世界模型里想象练习、把练好的策略放真环境跑。
