...
- 机器人之前要么靠语言指令（"把可乐罐拿过来"）、要么靠目标图（"最后画面长这样"）来知道该做什么。两种都对"新任务"水土不服。
- 这篇论文提出第三条路：**用一条画在摄像头画面上的 2D 曲线**告诉机器人"机械爪应该走这条路、在哪里夹一下、在哪里松开"。
- 训练时不用人工标注，把已有示教数据的末端轨迹自动反投影到摄像头画面就当 label 用，这一步叫 **hindsight trajectory labeling（事后轨迹打标签）**。
- 推理时这条曲线可以是人手画的、从人类示教视频里抠出来的、LLM 写代码生成的、甚至是图像生成模型画的。
- 在 7 个训练里没见过的新任务上，RT-Trajectory（2.5D 版）成功率 67%，同条件下 RT-1 是 17%、RT-2 是 11%、目标图版 RT-1-Goal 是 26%。差距非常大。
...
想象一只机器人手臂站在桌子前面，桌上摆着可乐罐、薯片袋、香蕉。它会做的事情就 8 类（捡、放近、立起来、推倒、开抽屉、关抽屉、放进容器、从容器拿出来），训练数据里这 8 类共 542 个具体任务、约 73K 条人类遥操作示教。

现在你让它做一件新的事：**把毛巾对折**。它训练时根本没见过"折毛巾"这三个字，也没见过"夹住毛巾一角拉到另一角"这个动作。但仔细想想，"拎起一角拉到另一边"这个动作和"把可乐罐挪到百事罐旁边"在机械爪的运动轨迹上其实非常像——都是抓起一个东西、移动一段、放下。

问题就在这：**任务描述用的是语言**。"折毛巾"和"挪可乐罐"在语言空间里完全不像，模型没法迁移已经学会的肌肉记忆。

类比一下：你教一个小朋友打羽毛球时学会的"挥拍动作"，他第一次打网球时其实大体动作是迁移过来的。但如果你只用文字描述（"打羽毛球"vs"打网球"），他可能反应不过来。如果你直接画一条手臂应该走的弧线给他看，他立刻就懂了。

RT-Trajectory 的核心观察：**轨迹曲线是比语言更接近"动作本身"的中间表达**。
...
**(b) Color Grading（颜色编码）**
- **红色通道**编码时间进度：值 = `(t+1)/T`，T 是这条示教的总长度。所以曲线一开始红色淡、结尾红色浓。这相当于把"速度"和"方向"画了进去。
- **绿色通道**编码高度：值 = `(h_{t+1} - h_min) / (h_max - h_min)`，归一化到 0~1。这是 "2.5D" 版本特有的——光看 2D 曲线分不清"机械爪是往里走还是往上抬"，把高度塞进绿色通道就能区分。

**(c) Interaction Markers（交互标记）**
...
骨架沿用 RT-1（一个 transformer-based 的 behavior cloning 模型）。
...
- 新增的 3 个输入通道在第一个卷积层的权重**初始化为全 0**，保证模型一开始相当于忽略 sketch（避免破坏预训练特征），训练过程中再学着用。
...
训练完了，现实场景给机器人一个新任务，曲线从哪儿来？论文给了 4 种来源：
