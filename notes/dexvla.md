...
DexVLA 在一个**冻结**（freeze，不更新参数）的视觉-语言模型（VLM）后面，挂一个 **1B 参数（10 亿参数）的 Diffusion Action Expert**（扩散动作专家）作为动作生成头，让一个本来只会"看图说话"的大脑，长出一只会精细动作的手。

这种"冻 VLM + 接动作专家"的拆解，是当前 VLA（Vision-Language-Action 视觉-语言-动作模型）路线里很重要的一支：**保住 VLM 的常识与泛化，把动作建模的脏活外包出去**。

## 这是个什么场景 — 日常类比

想象你请了一个**博览群书但没用过厨房刀的学者**（VLM），他能看一眼食材就告诉你"这是西红柿"，能听懂"切成丁"是什么意思，但你让他亲自动手切，他切出来的东西没法吃。

过去的做法是：**把学者拉去切菜训练营**（端到端 fine-tune 整个 VLM 学动作），结果学者切菜会一点了，但读书的本事退化了——这叫**灾难性遗忘**（catastrophic forgetting）。

DexVLA 的做法是：**学者继续读书不动手**（冻结 VLM），旁边请一个**动作专家**（1B Diffusion 模型），学者负责告诉专家"现在状况如何、用户想干嘛"，专家负责把这个意图翻译成连续的关节角度序列。两个角色分工，互不打扰。
...
1. **VLM 不动，只当"上下文翻译器"**——把图像 + 语言指令编码成一段隐表示（hidden representation），传给下游。冻结的好处：保留 VLM 在互联网级数据上学到的常识和指令理解能力，不会因为机器人数据规模小而退化。
2. **Diffusion Action Expert 做大做专**——1B 参数（具体数字需读原文确认是否包含 conditioning 部分）的扩散模型专门学"在 VLM 给定的语义条件下，生成符合机器人物理约束的动作轨迹"。规模够大，才能 cover 多任务、多 embodiment（本体形态）。
3. **多阶段训练课程**（curriculum）—— 据公开资料，DexVLA 强调分阶段训练：先在跨 embodiment 数据上预训练动作专家、再特化到目标本体、最后做任务级微调。这个 curriculum 是它能做到 dexterous（灵巧）操作的关键。

一句话总结这篇论文的"赌注"：**VLM 能力来自冻结预训练，动作能力来自规模化的扩散专家——两边各做各的，比硬塞进一个网络效果更好**。
