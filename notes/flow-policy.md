...
FlowPolicy 把"一致性流匹配"（Consistency Flow Matching, CFM）这种新一代生成模型嫁接到 3D 点云条件的机器人策略上，目标是让机器人既能像 Diffusion Policy 那样生成高质量、多模态的动作序列，又能把生成步数从几十步压到 1-2 步，从而在真机上跑得动、跑得快。

核心卖点有两个：

- 用一致性流匹配代替 DDPM/DDIM 那种多步去噪，推理一次/两次就能出动作
- 条件输入是 3D 点云（沿用 3D Diffusion Policy / iDP3 的路子），比 2D 图像更具几何感知能力

## 这是个什么场景 — 日常类比

想象你在教一个机器人"把杯子放到杯垫上"。

老的 Diffusion Policy 像是让机器人"先在脑子里画 100 张草稿图，每一张比上一张更清晰，最后看清第 100 张才动手"。结果是动作很稳，但每次决策都要在脑子里跑 100 步推理 — 慢。

Consistency Models 这一派想做的事是："能不能学个老师，让机器人看一眼模糊的草稿就直接跳到最终清晰图？" — 也就是把 100 步压缩成 1 步。

FlowPolicy 进一步说：连"草稿"这个概念都不要了，我们用"流匹配"（Flow Matching）— 直接学一条从噪声到动作的最短路径，再用"一致性"约束保证沿着这条路上的任何一点跳到终点都得到同样的答案。机器人因此可以一步直达。

而它看世界的方式不是 RGB 摄像头拍的平面图像，而是 3D 点云 — 类似你戴 AR 眼镜看到的"立体世界"，对手要去哪、杯子在哪一目了然。
...
**第一步：换底座 — 从 diffusion 换到 flow matching。**
Diffusion 的训练目标是预测噪声，推理时反向多步去噪。Flow Matching 直接学一个"速度场" v(x, t)：告诉你在中间状态 x、时间 t 时，应该往哪个方向走。理论上你只要沿着速度场积分（解 ODE），就能从噪声走到目标动作。Flow Matching 的训练比 diffusion 更直接，loss 是回归式的，没有复杂的 noise schedule。

**第二步：套上"一致性"约束 — 让 ODE 一步解。**
普通 Flow Matching 还是要多步 ODE solver。Consistency Flow Matching（CFM）加一个约束：从这条流的任何一点 x_t 出发，模型预测出的"终点动作"必须一致 — 不管你从 t=0.1 还是 t=0.9 跳，都得到同一个 x_0。这等价于学了一个"任意点都能直达终点"的函数，因此推理可以 1 步完成。

把这两步组合起来，再把"条件"接上 3D 点云编码器（沿用 DP3 的设计），你就得到了 FlowPolicy：3D 点云进，一步生成的动作序列出，质量接近多步 Diffusion Policy。

直觉上这是把 Consistency Policy 的"两阶段蒸馏"改成了"端到端单阶段训练"— 不再需要先训一个 teacher diffusion 再蒸馏，CFM 本身就把"一致性"作为训练目标，简化了流程。
