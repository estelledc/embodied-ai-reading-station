...
test("loadCanonicalNotes enumerates every repository note in slug order", () => {
  ...
  assert.equal(notes.length, 202);
  assert.equal(new Set(notes.map((note) => note.slug)).size, 202);
  ...
  assert.equal(notes.filter((note) => note.source.kind === "remote").length, 156);
...
test("production CLI --check exercises the tracked entry point without writing", () => {
  ...
  assert.match(output, /papers\/provenance\.json is current \(202 notes\)/);
  assert.deepEqual(fs.readFileSync(target), before);
});

test("tracked generator output passes the independent repository gate", () => {
  const result = validateProvenanceRepositoryFile({ root: REPO_ROOT, expectedNoteCount: 202 });
  assert.equal(result.ok, true, formatProvenanceRepositoryErrors(result.errors));
  assert.deepEqual(result.stats, {
    notes: 202,
    local_sources: 46,
    remote_sources: 156,
    generated_assets: 306,
    checked_paths: 554,
  });
});
