...
test("loadCanonicalNotes enumerates every repository note in slug order", () => {
  ...
  assert.equal(notes.length, 162);
  assert.equal(new Set(notes.map((note) => note.slug)).size, 162);
  ...
  assert.equal(notes.filter((note) => note.source.kind === "remote").length, 116);
...
test("production CLI --check exercises the tracked entry point without writing", () => {
  ...
  assert.match(output, /papers\/provenance\.json is current \(162 notes\)/);
  assert.deepEqual(fs.readFileSync(target), before);
});

test("tracked generator output passes the independent repository gate", () => {
  const result = validateProvenanceRepositoryFile({ root: REPO_ROOT, expectedNoteCount: 162 });
