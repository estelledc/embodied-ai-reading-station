...
test("loadCanonicalNotes enumerates every repository note in slug order", () => {
  ...
  assert.equal(notes.length, 178);
  assert.equal(new Set(notes.map((note) => note.slug)).size, 178);
  ...
  assert.equal(notes.filter((note) => note.source.kind === "remote").length, 132);
...
test("production CLI --check exercises the tracked entry point without writing", () => {
  ...
  assert.match(output, /papers\/provenance\.json is current \(178 notes\)/);
  assert.deepEqual(fs.readFileSync(target), before);
});

test("tracked generator output passes the independent repository gate", () => {
  const result = validateProvenanceRepositoryFile({ root: REPO_ROOT, expectedNoteCount: 178 });
  assert.equal(result.ok, true, formatProvenanceRepositoryErrors(result.errors));
  assert.deepEqual(result.stats, {
    notes: 178,
    local_sources: 46,
    remote_sources: 132,
    generated_assets: 162,
    checked_paths: 386,
  });
});
