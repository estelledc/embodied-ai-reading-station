 ...
 *
 * The script plans every note before it checks tools or writes. Existing assets
 * without provenance remain legacy inputs and are never overwritten implicitly.
 */
...
const DEFAULT_CARDS = path.join(ROOT, "site", "src", "images", "cards");
...
const RECIPE_VERSION = "fill-missing-cards-v2";
const GENERATOR_ID = "ffmpeg-fallback/fill-missing-cards/v2";
...
function cliOptions(argv = process.argv.slice(2)) {
  ...
  return {
    ...
    outputDir: path.resolve(values["output-dir"] || DEFAULT_CARDS),
...
function buildPlan({ outputDir, slugs, manifest }) {
  ...
  for (const file of files) {
    ...
    const source = cardSrc(slug, data.topic || "");
    const unsafeSource = Boolean(source && !isRegularRepositoryFile(source));
    const full = path.join(outputDir, `${slug}.webp`);
    const compact = path.join(outputDir, `${slug}-800.webp`);
    ...
    if (!manifestRecord) {
      ...
      reason = "no paper, inline-scene, or topic source";
...
function finalizePlan(plan, contentCommit, ffmpegVersion, options) {
  const checks = plan.filter((entry) => entry.state === "candidate-check");
  let receipt = null;
  if (checks.length > 0 && options.slugs.size === 1 && options.receiptFile && fs.existsSync(options.receiptFile)) {
    ...
    validateCardReceipt(receipt, options);
  }
  return plan.map((entry) => {
    if (entry.state !== "candidate-check") return entry;
    const outputFacts = entry.outputs.map((output) => ({ entry, output, image: inspectImage(output.path) }));
    const outputs = entry.outputs.map((output) => ({ ...output, sha256: outputFacts.find((fact) => fact.output === output).image.sha256 }));
    if (!receipt || receipt.slug !== entry.slug) {
      return { ...entry, outputs, state: "error-receipt-required", reason: "registered assets require their portable receipt for input verification" };
    }
    const inputs = receiptInputs([entry], {
    ...
    return current
      ? { ...entry, outputs, state: "skip-current", reason: "receipt fingerprint and output hashes match provenance" }
      : { ...entry, outputs, state: "error-recorded-drift", reason: "recorded bytes or generator inputs drifted; refusing overwrite" };
  });
}
...
function receiptInputs(entries, { contentCommit, ffmpegVersion }, outputFacts) {
  return {
    ...
    prompt_sha256: null,
    template: { id: "fill-missing-cards", version: RECIPE_VERSION },
    generator: { id: GENERATOR_ID, version: ffmpegVersion },
    converter: { id: "ffmpeg", version: ffmpegVersion },
    parameters: {
      codec: "libwebp",
      pair_atomic: true,
      source_priority: ["paper-img-000", "inline-scene", "topic"],
    },
...
function validateCardReceipt(receipt, options) {
  const [slug] = options.slugs;
  if (receipt.slug !== slug) throw new Error(`receipt slug ${receipt.slug} does not match --slug ${slug}`);
  if (receipt.generator !== GENERATOR_ID) throw new Error("receipt was not created by fill-missing-cards");
  const expected = new Set([
    repoPath(path.join(options.outputDir, `${slug}.webp`)),
    repoPath(path.join(options.outputDir, `${slug}-800.webp`)),
  ]);
  if ([...expected].some((value) => !value)) throw new Error("--record output directory must be inside the repository");
  if (receipt.outputs.length !== 2 || receipt.outputs.some((output) => output.kind !== "card" || !expected.has(output.path))) {
    throw new Error("receipt must contain the exact card full/800 pair");
  }
}
...
function printPlan(plan, { ffmpegVersion }) {
  ...
  for (const entry of plan.filter((item) => !item.state.startsWith("skip-"))) {
    console.log(JSON.stringify({ slug: entry.slug, state: entry.state, source: entry.sourceRepoPath, outputs: entry.outputs.map((item) => item.repoPath) }));
  }
}
...
function generationRequestError(candidates, options, { allowExternal = false } = {}) {
  ...
  if (!allowExternal && candidates.flatMap((entry) => entry.outputs).some((output) => !/^site\/src\/images\/cards\/[a-z0-9-]+(?:-800)?\.webp$/.test(output.repoPath || ""))) {
    return "receipt-backed card generation requires the canonical repository output directory";
  }
...
async function main(options) {
  ...
  if (errors.length > 0) {
    console.error(`FAIL ${errors.map((entry) => `${entry.slug}: ${entry.reason}`).join("; ")}`);
    process.exitCode = 1;
    return;
  }
  ...
  if (candidates.length === 0) {
    console.log("\n=== no card pairs to generate ===");
    return;
  }
  const generated = generateEntries(candidates, options, { contentCommit, ffmpegVersion });
  console.log(`\n=== generated ${generated.results.length} card assets; receipt ${generated.receipt.file} ===`);
}
